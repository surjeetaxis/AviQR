...
import in.aviqr.order.repository.*;
...
public class QrService {
    ...
    @Transactional
    public in.aviqr.order.entity.QrCode createMarketing(String label, String targetUrl, String campaign) {
    ...
    @Transactional
    public in.aviqr.order.entity.QrCode updateMarketing(UUID id, String label, String targetUrl, String campaign) {
        in.aviqr.order.entity.QrCode qr = repo.findById(id)
    ...
    public List<in.aviqr.order.entity.QrCode> getByShop(String shopId) {
    ...
    private String buildUrl(String shopId, QrType type, String group) {
        ...
        return switch (type) {
            ...
            // shopId is a synthetic "hotel-{hotelId}" id — the whole-hotel QR (lobby/front-desk),
            // same GuestServices.jsx route as HOTEL_ROOM but with no room param, so Requests/Bill
            // prompt the guest to scan their room's own QR instead.
            case HOTEL        -> baseUrl + "/hotel-services/" + shopId.replaceFirst("^hotel-", "");
