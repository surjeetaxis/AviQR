...
describe('authApi', () => {
  ...
  it('login posts to /api/v1/auth/login with credentials', async () => {
    await authApi.login({ email: 'a@b.com', password: 'Test@1234' });
    expect(m.post).toHaveBeenCalledWith('/api/v1/auth/login', { email: 'a@b.com', password: 'Test@1234' });
  });

  it('register posts to /api/v1/auth/register', async () => {
    const body = { name: 'X', email: 'x@y.com', phone: '9900112233', password: 'Test@1234', role: 'CUSTOMER' };
