...
import { planApi, offerApi, mallApi } from '../../src/api/index.js';
import { PageHeader } from '../../src/components/common/PageHeader.js';
import { Colors, FontSize, Spacing, Radius, Shadow } from '../../src/theme/index.js';

// Plan browsing/upgrading is web-only — the app never runs a subscription
// purchase flow. Mobile just shows whatever plan the mall is actually on
...
export default function MallSubscriptionScreen() {
  ...
  useEffect(() => {
    Promise.all([mallApi.getMyMalls(), planApi.listPublic('MALL'), offerApi.listActive()])
      .then(([mRes, pRes, oRes]) => {
        setMall((mRes.data.data || [])[0] || null);
  ...
  const currentPlan = plans.find(p => p.planKey === mall?.subscriptionPlan);
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.base, paddingBottom: 40 }}>
        {loading ? <Text style={ss.loading}>Loading…</Text> : currentPlan ? (
          ...
          <View style={ss.card}>
            ...
            <Text style={ss.feature}>Subscribe on the web to unlock the mall module.</Text>
          </View>
        )}
