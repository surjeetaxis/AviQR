...
// (confirm() + local state, no backend call), so nothing real to wire beyond browsing.
export default function HotelSubscriptionScreen() {
  ...
  useEffect(() => {
    Promise.all([planApi.listPublic('HOTEL'), offerApi.listActive()])
    ...
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.base, paddingBottom: 40 }}>
        {loading ? <Text style={ss.loading}>Loading…</Text> : plans.length === 0 ? (
          <Text style={ss.loading}>No hotel plans configured yet.</Text>
        ) : plans.map(plan => {
